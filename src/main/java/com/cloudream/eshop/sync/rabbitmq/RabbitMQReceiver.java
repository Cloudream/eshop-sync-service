...
public class RabbitMQReceiver {
	...
	private static List<JSONObject> brandBatchDataMessageList = new ArrayList<JSONObject>();
	...
	private void processBrandSyncChangeMessage(JSONObject messageJSONObject) {
		...
		if (OperationConstant.ADD.equals(eventType) || OperationConstant.UPDATE.equals(eventType)) {
			brandBatchDataMessageList.add(messageJSONObject);
			System.out.println("将品牌数据放入内存List中，list.size = "+ brandBatchDataMessageList.size());
			if (brandBatchDataMessageList.size() >= 3) {
				System.out.println("[开始批量处理]，list.size大小等于" + brandBatchDataMessageList.size());
				String ids = "";
				for (int i = 0,len = brandBatchDataMessageList.size();i < len ;i ++) {
					ids += brandBatchDataMessageList.get(i).getLong("id");
					if (i < brandBatchDataMessageList.size() -1) {
						ids += ",";
					}
				}
				System.out.println("[要处理的品牌数据ids]，ids=" + ids);
//				JSONObject dataJSON = JSONObject.parseObject(eshopProductService.findBrandById(id));
				...
				System.out.println("[批量调用获取到的品牌数据]，JSONArray=" + brandJSONArray.toJSONString());
				for (int i = 0,len = brandJSONArray.size();i<len;i++) {
					JSONObject dataJSON = brandJSONArray.getJSONObject(i);
					Jedis jedis = jedisPool.getResource();
					jedis.set("brand_" + dataJSON.getLong("id"), dataJSON.toJSONString());
					System.out.println("[将品牌数据写入redis]，brandId=" + dataJSON.getLong("id"));
					dimDataChangeMessageSet.add("{\"dim_type\":\"brand\",\"id\":" + dataJSON.getLong("id") + "}");
					System.out.println("[将品牌数据写入内存去重Set中]，brandId=" + dataJSON.getLong("id"));
				}
				
				brandBatchDataMessageList.clear();
			}
			...
			while (true) {
				if (!dimDataChangeMessageSet.isEmpty()) {
					for (String message : dimDataChangeMessageSet) {
						rabbitMQSender.send(RabbitQueueConstant.DATA_SYNC_CHANGE_QUEUE, message);
